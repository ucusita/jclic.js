...
export class Activity {
            ...
            switch (child.nodeName) {
                case 'settings':
                    ...
                    $node.children().each((_n, child) => {
                        const $node = $(child);
                        //console.log('child.nodeName en $node.children().each (Read elements of _settings_) contiene:', name);
                        switch (child.nodeName) {
                            case 'skin':
     ...
     */
    prepareMedia(ps) {
        //console.log('prepareMedia(playStation) (474)', ps);
...
export class ActivityPanel extends Container {
    ...
    /**
     * Play TTS
     * @param {string} msg - Mensaje a reproducir por TTS
     * @param {int} delay - retardo en segundos antes de reproducir el texto
     */
    playTTS(msg, delay) {
        // if (this.playing) {
        //     setTimeout(() => play(msg, 'es-ES', 1), delay * 1000);
        // } else {
        //     play(msg, 'es-ES', 1);
        // }        
        ...
        //Speech.voiceURI = "Google español de Estados Unidos";
        Speech.volume = 1; // 0 to 1
        Speech.rate = 1; // 0.1 to 10
        Speech.pitch = 0; //0 to 2
        Speech.lang = 'es-US';
        setTimeout(() => speechSynthesis.speak(Speech), delay * 1000);
